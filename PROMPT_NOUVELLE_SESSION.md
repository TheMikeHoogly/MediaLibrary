...
**Branche `integration`** = `main` (correctifs tagging) + file-ops + control-center,
pour tout tester d'un seul redemarrage. Les deux features vivent aussi sur leur
propre branche (`feat/file-ops`, `feat/control-center`) pour merge separe.
- **File-ops (gestion de fichiers /browse) : COMPLET.** `fichiers.py` (pur,
  confinement + cle + quarantaine + undo) + `test_fichiers.py` 23/23 ; routes
  `/api/files/rename|move|mkdir|delete|undo` (`_do_files_post`, `file_ops()`),
  re-cle via `rekey_everywhere` (zero nom perdu) ; UI `BROWSE_PAGE` (rangees
  selectionnables + barre papier couper/coller/supprimer-quarantaine/nouveau
  dossier/annuler, `__CTX__`). **RESTE : valider en reel puis merger dans `main`.**
- **Centre de controle `/reglages`** : COMPLET. Onglet nav « Reglages » ; dashboard
  ...
  `MAINT_PAUSED`), `/census` (recensement lecture seule). `GET /api/maint/status`.
  Rien de destructif en un clic. **RESTE** : valider ; ensuite exposer l'application
  du plan de rangement (garde-fous) et l'edition des reglages (lecture seule pour l'instant).
