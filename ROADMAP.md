...
## État (16/08/2026, session 16)

**Banc 3b TRANCHÉ — la re-passe de tagging est CLOSE.** 147 paires notées à
l'aveugle. V2CTX préféré **94/147 (63,9 %, p = 0,0009)**, largement au-dessus du
seuil pré-enregistré de 86 — **mais ses hallucinations ont doublé** : 24 contre
13, et 15 contre 4 en apparié (p = 0,019). Le critère écrit d'avance est un ET,
et sa branche basse dit « hallucinations en hausse → close ». Deux nuances qui
comptent : **hors des 30 pièges** (documents, reçus, captures) la préférence
tombe à 69/117 (59,0 %, p = 0,064) — **sous le seuil** ; et la strate
« incertain » est à 10-10. Le gain était dans les faits, pas dans la
...
**14a livré ET OBSERVÉ EN RÉEL.** `recherche.py` (pur, testé hors serveur comme
`meme_jour` ; 48 tests) décompose : noms → lieux → **période** → reste à SigLIP.
Mesuré sur le serveur : « années 80 » rend **752 photos** (les 716 rendues en
session 14 sont retrouvables) ; « avant 2000 » et « depuis 2000 » sont
**disjoints** ; « la photo a été prise à Lausanne » ne confond pas le participe
passé avec la saison. Le lieu GÉOCODÉ branché sur la recherche multiplie la
matière : **Lausanne 120 → 1 031**, Bremblens 303 → >1 500, Sion 14 → 21.
Deux précisions jamais mélangées, et le filtre **compte** ce qu'il écarte —
`sans_date` = **3 824** (aucune date au jour près) ou **260** (aucune année
fiable), les deux confirmés par un second chemin de code sur une COPIE de la
base.

**Vecteurs sémantiques orphelins — MESURÉ, à traiter.** `/api/search` remonte
**2 374 photos absentes de l'index** (`vectors` kind `photo` sans ligne `tags`) :
résultats MUETS, ni description ni mots-clés, **2,6 % des résultats** sur huit
requêtes ordinaires. Ventilation : **2 143 = ARZOPA** (dossier supprimé le
08/08, vecteurs jamais purgés), 91 en `.corbeille-rangement`, 69 clés
malformées. **Aucun n'a de jumeau présent dans `tags`** — les purger ne perd
rien d'indexé. Diagnostic permanent : `verifier_orphelins.py` (base contre
base, zéro accès disque, fiable NAS débranché).
...
2. **Observer en réel ce qui est livré** — **fait ✔** (14a compris). Reste :
...
14. **Recherche IA locale contextuelle (demandé 12/08).** (a) **Déterministe —
    LIVRÉ ET OBSERVÉ 15/08** (État). Manques : les `faits` ne filtrent pas
    encore (le lieu passe par `gps_places` + chemin) ; pas de filtre espèce ni
    fiche ; le tri sans mot-clé reste `_best_time` (donc `mtime`) là où la
    sélection l'exclut ; **purger les 2 374 vecteurs orphelins**. (b) ensuite
...
Vidé le 12/08 : trois résiduels rattachés en wagons aux chantiers 10 et 11e.
**Corrigés le 15/08** : `sys.argv[1]` pris pour `UPLOAD_DIR` (un drapeau n'est
pas un dossier) et l'import qui MOURAIT quand sa sortie est un tuyau
(`UnicodeEncodeError` sur un pictogramme en cp1252 — `python outil.py > log.txt`
était mortel). Test unique, 6 vérifications.
...
< 1990 lue dans le NOM DE FICHIER, ce qui coûte **7 photos** (« 19861220 … »,
sans date EXIF, aujourd'hui datées par leur dossier). Les deux sont
