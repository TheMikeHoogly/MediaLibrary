...
| Garde humain/animal auto (SigLIP zéro-shot) | **REJETÉ** (08/08) | Visages et statues/chats se chevauchent : 18 % faux rejets. Remède : action « C'est un animal ». |
...
| Détecteur ML de rebut / flou auto | **REJETÉ par conception** (03/08) | Rebut évident = règle simple ; subtil non isolable sans risquer une bonne photo. Le subtil reste à l'humain. |
| `sqlite-vec` ; embeddings INT8 | **REJETÉ** (11/08) | Cosinus numpy sur BLOB suffit. INT8 : ×2 seulement (159→80 Mo), recall@10 0,9685 — « sans perte » réfuté. |
...
| **Re-passe complète de tagging (~50 h GPU)** | **CLOSE** (16/08, banc 3b) | 147 paires en aveugle. V2CTX préféré **94/147 (63,9 %, p = 0,0009)** — **mais hallucinations doublées (apparié 15-4, p = 0,019)**, et **hors des 30 pièges 69/117 (59,0 %), sous le seuil**. Le critère est un ET ; tout l'écart venait des documents. |
...
| Faits en contexte pour DOCUMENTS/reçus/captures | **HYPOTHÈSE** (16/08) | Strate « piège » : **25/30 (83 %, p = 0,0003)**, la seule qui passe — mais POST-HOC sur 30 photos : une hypothèse, pas une décision. |
...
| Idée / piste | Verdict | Raison |
|---|---|---|
| Appliquer les 7 058 renommages | **FAIT** (17/08) | 36 lots, **0 sauté**, undo = 7 058 opérations. Plan régénéré après : **0 à renommer**. Restent 199 noms bruts (192 `failed`). Noms humains intacts. |
| −250 entrées en mémoire pendant les lots | **TRANCHÉ, mécanisme INCONNU** (17/08) | Mémoire 43 064 → **42 814**, `photos.db` resté à 43 064 ; au redémarrage, 43 064. **Rien de perdu** ; la mémoire a été fausse ~15 min. Ce n'est PAS `forget_everywhere` (`pending` = 2). Non identifiable après coup : rien ne compte ce que le scan oublie. |
| Chercher le mécanisme sans instrument | **REJETÉ** (17/08) | Trois hypothèses testées et tombées. La suite est de l'INSTRUMENTATION, pas de la déduction. |
| Le repli sur le NOM est gardé comme `taken` | **FAUX — OUVERT** (17/08) | Étape 1 refuse `taken` ; l'étape 2 lit la date du NOM **sans contrôle** : un scanner qui nomme `20150810_073417.jpg` réinscrit la date écartée. **1 cas**, mais une porte sur deux. |
...
| Laisser les vecteurs des photos sorties de l'index | **TRAITÉ** (17/08) | 2 374 vecteurs purgés, quarantaine réversible. Après redémarrage : **0 muet sur 1 600 résultats** (2,6 % avant). |
...
| Portée des dates de SCAN en base | **MESURÉE : 72** (17/08) | `mesure_dates_scan.py` (PUR, 32 tests, lecture seule sur COPIE) applique à `taken` le critère du renommage : **72** contre 12 connues — le plan ne voyait que les noms bruts. Presque toutes dans « Photos Papa » ; écarts +2 à +32 ans ; 0 `failed`. Confirmé sur le serveur VIVANT (`/api/jour?jour=05-01` rend les 4 photos de `1990_Achumani`, `precise: true`). Angle mort compté : 6 818 sans année de dossier, 10 226 sans `taken`. |
| Corriger `taken` en BASE | **NON DÉCIDÉ** (17/08) | La mesure est faite, la correction non : elle touche le pipeline de dates (`monolith-surgery`) et exige un backfill, pour 72 photos — face à 1 369 dates antérieures à ne pas emporter. Deux gestes moins chers d'abord : garder l'étape 2 du repli, rendre au plan les 15 noms périmés. |
