...
| **Re-passe complète de tagging (~50 h GPU)** | **CLOSE** (16/08, `docs/PROTOCOLE_3B_TAGGING.md`) | 147 paires en aveugle : V2CTX préféré **63,9 %** mais **hallucinations doublées** (15-4) et **59,0 % hors des 30 pièges**, sous le seuil. Le critère est un ET ; l'écart venait des documents. |
...
| Idée / piste | Verdict | Raison |
|---|---|---|
| Appliquer les 7 058 renommages | **FAIT** (17/08) | 36 lots, **0 sauté**, undo complet, noms humains intacts. Plan régénéré : **0 nom brut**. |
| Chercher le mécanisme des −250 sans instrument | **REJETÉ → instrument OBSERVÉ** (18/08) | Rien n'était perdu (mémoire fausse ~15 min, base intacte), et rien ne comptait : `comptes_index.py` compte désormais au GOULOT (`TrackedDict`). **Contrôle positif en réel** : +1 (`tagging`) puis −1 (`scan:disparus`), `inexpliqué` 0 ; 12 cycles au repos à 0. S'il revient, le registre rendra le chiffre. |
| Le repli sur le NOM est gardé comme `taken` | **CORRIGÉ** (19/08) | Le numériseur écrit l'instant du scan dans `DateTimeOriginal` **et** dans le nom : le garde-fou du 17/08 ne fermait qu'une porte sur deux. **73** noms refusés = **1** vraie date de scan + **72** faux futurs, **0 nom brut** : le geste ferme une porte sans déplacer un fichier, et c'est le résultat. |
| Le plan revient sur ce qu'il a renommé | **CORRIGÉ, OBSERVÉ** (19/08) | `est_nom_annee_seule` rend au plan les `YYYY0000_` qu'il a écrits, **seulement si la date est devenue précise** (sinon le compteur de collision se rebrasse pour rien). **15** moves — le compte exact de l'AUTRE chemin — appliqués en réel : 15 cibles distinctes, 0 scan réinscrit, noms humains intacts, plan régénéré à **0**. **376** attendent encore une date. |
...
| Deux règles de date dans une même réponse | **CORRIGÉ** (19/08) | La recherche FILTRAIT par `annee_fiable` (jamais `mtime`) puis TRIAIT par `_best_time`, dont la branche 3 est le `mtime`. Sur copie : **259** photos sans date sûre, **257** datées de 2026 par leur propre tagging, **en tête** de **56 des 364 noms** (53/100 pour « Véronique »). Et **32** sans même un `mtime` : `… or ''` mélangeait `float` et `str` — l'ancien tri **ne s'exécute pas** sur l'index entier (TypeError → 500), sans qu'aucun NOM ne le déclenche (0/364, le chemin par LIEU non mesuré). `recherche.trier_chronologique` : une seule règle, sans-date en FIN et comptées. **Reste à observer.** |
...
| Portée des dates de SCAN en base | **MESURÉE : 72** (17/08) | `mesure_dates_scan.py` (PUR, sur COPIE) applique à `taken` le critère du renommage : **72** contre 12 connues, presque toutes dans « Photos Papa », +2 à +32 ans. Confirmé sur le serveur VIVANT. Angle mort : 6 818 sans année de dossier, 10 226 sans `taken`. |
...
| Plafond 2100 de la date lue dans un NOM | **PARKÉ chiffré** (19/08) | `22082010141.jpg` (DDMMYYYY) se lit « 2082-01-01 » : **72** en base, **coût 0** — mais seulement parce qu'elles ont un `taken`. Détail : Résiduels du ROADMAP. |
