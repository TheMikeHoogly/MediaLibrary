...
| Deux règles de date dans une même réponse | **CORRIGÉ, OBSERVÉ** (19/08) | La recherche FILTRAIT par `annee_fiable` (jamais `mtime`) puis TRIAIT par `_best_time`, dont la branche 3 est le `mtime`. Sur copie : **259** photos sans date sûre, **257** datées de 2026 par leur propre tagging, **en tête** de **56 des 364 noms**. Et **32** sans même un `mtime` : `… or ''` mélangeait `float` et `str` — l'ancien tri **ne s'exécute pas** sur l'index entier (TypeError → 500), sans qu'aucun NOM ne le déclenche (0/364 ; chemin par LIEU non mesuré). En réel : `sans_date_tri` = 53 · 43 · 29 · 29 · 21 (Véronique, Nikola, Mike, Marie, Sandra), au chiffre près la mesure. |
