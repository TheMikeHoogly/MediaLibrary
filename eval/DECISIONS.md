...
| **Livrer dans git depuis la sandbox** | **ADOPTÉ, OBSERVÉ** (19/08) | `git` lancé depuis la VM laisse un `.git/index.lock` qu'elle ne sait pas supprimer : une impossibilité, pas un principe. Un agent WINDOWS lance le vrai git ; premier acte observé, il s'est commité lui-même (`a43f9d0`). |
| Libérer le port PUIS faire les préparatifs lents | **CORRIGÉ** (19/08) | 40 s de `pip` entre le kill et le démarrage : un orphelin qui se levait reprenait le port. Le lent passe AVANT ; port tenu après 10 essais → on ne lance RIEN. |
| `taskkill` par TITRE DE FENÊTRE retire l'ancien superviseur | **FAUX** (20/08) | Il ne retrouve pas fiablement les fenêtres console : il n'a RIEN tué. L'ancien superviseur a survécu, a pris pour un plantage le kill par PID de son serveur et en a relancé un second — **deux fenêtres « MediaLibrary - Serveur »**. Remplacé par une GÉNÉRATION : le bat 0 écrit un jeton neuf, chaque superviseur le relit et se retire s'il a changé. Un titre se devine, un fichier se lit. |
| Une fenêtre d'agent minimisée | **REJETÉ** (19/08) | Morte-née, elle était indiscernable d'une fenêtre en écoute. Fenêtre VISIBLE, `_agent_git_vu.txt` touché à chaque tour, `ping` inerte. |
| L'agent est un bouton déporté | **REJETÉ** (19/08) | Il est la **porte** : refus tant que le serveur ne fait pas tourner le code visé, qu'un test est rouge, qu'un `.bat` n'est pas ASCII ou que le lint crie. D'où l'ordre **inversé**. |
...
| **Segments entiers = la bonne règle pour ce qu'on VOIT** | **FAUX, et c'est le résultat du chantier** (19/08) | Sur 876 lieux « collés », ~330 sont VRAIS — « Yani2004 » (219). D'où la découpe des mots sur les frontières de CASSE et de CHIFFRES : « Vallorbe » reste entier, « Cousins&Cousines » ne rend jamais « Ins ». |
| **Règle de lieu unifiée sur ses trois appelants** | **ADOPTÉ, OBSERVÉ** (19/08) | `places_list` et `_cles_du_lieu` délèguent à `faits_vue.lieux_du_chemin` (`tous=True` : une photo compte dans CHAQUE lieu). « Ins » **493 → 5** ; recherche **499 → 11 dont 0** de « Cousins&Cousines ». Gains : Sud France 315, San Borja 82, Vallée d'Aoste 81. Page **2 119 → 1 539 ms**. |
| Le NOM DE FICHIER comme source de lieu | **ADOPTÉ, limite assumée** (19/08) | 132 paires dont **71 déjà couvertes par le GPS**. Sur les 61 restantes : **52 vraies** contre **9 fausses** qu'aucune règle syntaxique n'attrapera (« Grupo en la Laguna »). |
...
| **Faits AFFICHÉS depuis la vue (planche + visionneuse)** | **ADOPTÉ, OBSERVÉ** (19/08) | Un seul producteur client (`faitsHtml`), un seul assembleur serveur (`faits_vue.assertions`), les quatre modes de `/files` partagés. `q=Ins` : 11 photos — 11 dates, 11 lieux, 5 noms ; page **1 048–1 462 ms** contre 1 539. |
| **Index inversé des noms, en DEUX passes** | **ADOPTÉ, MESURÉ** (19/08) | Page de 50 : **1,11 ms** contre **9,65 ms** naïf (**×8,7**). Une passe unique est REJETÉE : un retrait posé par une fiche vue APRÈS celle qui attribue ne retirerait rien. Six tests. |
...
| **Le FILTRE des noms lit les `kw` bruts** | **CORRIGÉ, OBSERVÉ** (20/08, 14a-iv) | L'affichage lisait les fiches, le filtre l'index : deux chemins, deux réponses. **13 photos** rendues alors qu'`exclude` avait retiré le nom (Mike 6, Flo 5, Silvio 1, Danica 1), **0** dans l'autre sens — 363 tags balayés sur copie. `_autorite_des_noms` : une implémentation, deux appelants. Observé : Silvio **495 → 494**, Danica **325 → 324**. |
| La FICHE fait foi sur l'ORTHOGRAPHE | **ADOPTÉ, OBSERVÉ** (20/08) | L'index porte des mots-clés écrits avant elle : « Luna · luna » (2 photos, le même chat deux fois) et « luna » seul (1). La graphie change, jamais le nom — la recherche compare en minuscules. |
| **Portée du filtre déterministe : 92,74 %** | **MESURÉ** (20/08) | Sur les **30 122** photos à fait NON-date, nom ou lieu en atteint **27 936**. Les **2 186** autres n'ont qu'une ESPÈCE : hors de portée, SigLIP seul les sort. Le 5ᵉ axe est une question à Mike. |
...
| Le garde-fou de la date de SCAN passe à la LECTURE | **ADOPTÉ, OBSERVÉ** (19/08) | `faits_vue.date_credible` INJECTÉ dans `meme_jour.epoch_precis` : une implémentation pour le tri, le filtre, « même jour » et le fait. **70** photos perdent une date précise fausse, observé sur `Photos Papa\1983\20150810_…`. |
...
| Idée / piste | Verdict | Raison |
|---|---|---|
| Les noms répétés sous la visionneuse | **RETIRÉS, OBSERVÉ** (19/08, choix de Mike) | La ligne de faits les dit déjà, triés, sans préfixe, avec leur source. État vide RÉDIGÉ : « aucun mot-clé au-delà des noms » — « pas encore de tags » mentirait sur une photo nommée. Le FILTRE de la planche les garde. |
...
| Idée / piste | Verdict | Raison |
|---|---|---|
| Sortir les verdicts clos dans une archive | **REJETÉ** (19/08, choix de Mike) | Précédent de `METHODE.md` (16/08), mais deux fichiers valent moins qu'un seul complet : « ne rien re-proposer » suppose de tout relire d'un coup d'œil. |
| **Budget porté de 9 000 à 12 000 octets** | **ADOPTÉ** (19/08, choix de Mike) | À 8 969 pour 9 000, la seule marge restante était la PRÉCISION des raisons — or c'est elle que le seuil protégeait. Le seuil garde son rôle contre le récit ; le détail reste dans git. |
