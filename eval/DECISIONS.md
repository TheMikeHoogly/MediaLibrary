...
| −250 entrées en mémoire pendant les lots | **TRANCHÉ, mécanisme INCONNU** (17/08) | Mémoire 43 064 → **42 814**, `photos.db` resté à 43 064 ; au redémarrage, 43 064. **Rien de perdu** ; la mémoire a été fausse ~15 min. Ce n'est PAS `forget_everywhere` (`pending` = 2). Non identifiable après coup — rien ne comptait alors. |
| Chercher le mécanisme sans instrument | **REJETÉ → instrument livré** (17→18/08) | Trois hypothèses testées et tombées ; la suite était de l'INSTRUMENTATION. `comptes_index.py` compte au GOULOT (`TrackedDict`) : motif par appelant, bucket « non déclaré », et réconciliation par cycle (`inexpliqué = (fin−début) − (ajouts−retraits)`). Les −250 auraient rendu un chiffre. **Non encore observé en réel** — `/reglages`. |
