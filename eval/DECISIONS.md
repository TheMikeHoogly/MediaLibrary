...
| Le plan revient sur ce qu'il a renommé | **CORRIGÉ, OBSERVÉ** (19/08) | `est_nom_annee_seule` rend au plan les `YYYY0000_` qu'il a écrits, **seulement si la date est devenue précise**. **15** moves — le compte exact de l'AUTRE chemin — appliqués : 0 scan réinscrit, noms intacts, plan régénéré à **0**. **376** attendent une date. |
...
| Portée des dates de SCAN en base | **MESURÉE : 72** (17/08) | Le critère du renommage appliqué à `taken` (sur COPIE) : **72** contre 12 connues, presque toutes dans « Photos Papa », +2 à +32 ans. Confirmé sur le serveur VIVANT. Angle mort : 6 818 sans année de dossier, 10 226 sans `taken`. |
